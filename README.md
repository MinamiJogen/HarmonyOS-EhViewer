...
- 重构下载页为“已下载 / 下载中”双页签：已下载使用瀑布流画廊卡片，下载中保持任务列表并从顶部开始布局，内容不足一屏也可滚动。
- 修复下载任务重新进入页面后状态、单任务进度条、总进度条、开始/暂停控制和移除状态不同步的问题。
- 下载进度退回通知中心展示，同一画廊只更新同一条通知，点击通知会直接进入下载中页面。
- 调整首次登录、本地账号缓存和设置页删除本地账号缓存逻辑，默认功能优先使用本地缓存。
- 移除模拟器调试代理设置入口，默认只使用系统代理能力，不在正式界面暴露本地代理开关。

### 1.0.3 (2026-06-24)

- 新增“我的 - 设置 - 隐私保护”，支持锁定 App，进入前台时使用面容、指纹或锁屏密码验证，默认关闭。
- 新增后台预览保护开关，默认开启，用于进入后台后的最近任务预览保护，不影响正常前台截图和录屏。
- 优化锁定 App 的开关刷新、认证等待页、前后台恢复和失败重试逻辑，减少设置页进入时的卡顿和错位。
- 详情页会根据阅读器退出时页码显示“继续阅读”和“从 xx 页”，并提供重新阅读入口。
- 整理 ArkTS 文件结构，拆分隐私、设置解析、阅读进度和通用工具模块，降低 `Index.ets` 的单文件负担。
